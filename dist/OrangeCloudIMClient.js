...
class OrangeCloudIMClient {
    ...
    buildConnection(hubUrl, appId, userId, userSig) {
        ...
        return new signalr_1.HubConnectionBuilder()
            ...
            // 不使用库内置 withAutomaticReconnect：四端统一采用 SDK 自管理的手动重连
            // （手动重连支持 maxReconnectAttempts、onReconnectAttempt 事件、网络恢复立即重连、重连后携带 lastSequenceNumber 重新加入群组）
    ...
    registerCallbacks() {
        ...
        // 群成员列表事件（getGroupMemberList 的响应，独立事件避免与普通消息混用）
        this.connection.on("ReceiveGroupMemberList", (json) => {
            this.options.onGroupMemberList?.(json);
        });
    }
    registerConnectionEvents() {
        if (!this.connection)
            return;
        // 统一手动重连：连接断开后由 attemptReconnect 处理。
        // 不再依赖库的 onreconnecting/onreconnected（已禁用 withAutomaticReconnect）。
